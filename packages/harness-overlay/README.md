...
Product subagents follow the Profile Bundle contract in the pinned `dsh-v0.1.2-alpha.3` runtime and are not embedded in Minke's base runtime. Install one into the `web` Profile:
...
- alpha.3's native whole-session turn outline and deep-history jump loader, so unloaded turns remain previewable and navigable without a Minke-owned conversation directory;
