...
Agent replies use the provider-specific `rich-markdown` delivery intent and Telegram's `sendRichMessage` method. The original GFM-like Agent output is passed as `rich_message.markdown`, which supports headings, lists, tables, block quotes, fenced code, and inline formatting without MarkdownV2 rewriting or escaping. Pairing, health, and failure messages remain plain `sendMessage` deliveries, so Telegram formatting policy stays at the provider boundary instead of leaking into the shared Agent route or IM Gateway. A structured 400 or 404 rejection has a definite remote effect of `none` and may fall back once to plain text when the reply fits `sendMessage`; timeouts, network failures, malformed responses, cancellation after send, and every other uncertain outcome never trigger a second send. See Telegram's official [`InputRichMessage`](https://core.telegram.org/bots/api#inputrichmessage), [Rich Markdown formatting](https://core.telegram.org/bots/api#rich-message-formatting-options), and [`sendRichMessage`](https://core.telegram.org/bots/api#sendrichmessage) references.
